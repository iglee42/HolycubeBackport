...
import fr.iglee42.holycubebackport.blocks.HangingMossBlock;
...
public class PaleMossDecorator extends TreeDecorator {
    ...
    protected TreeDecoratorType<?> type() {
        return HolycubeBackport.PALE_MOSS_DECORATOR;
    }
        ...
        if (!list.isEmpty()) {
            ...
            if (randomsource.nextFloat() < this.groundProbability) {
                worldgenlevel.registryAccess()
                    ...
                    .flatMap(p_382782_ -> p_382782_.get(HolycubeBackport.PALE_MOSS_PATCH))
                    .ifPresent(
    ...
    private static void addMossHanger(BlockPos p_379694_, TreeDecorator.Context p_379973_) {
        while (p_379973_.isAir(p_379694_.below()) && !((double)p_379973_.random().nextFloat() < 0.5)) {
            p_379973_.setBlock(p_379694_, HolyBackBlocks.PALE_HANGING_MOSS.defaultBlockState().setValue(HangingMossBlock.TIP, Boolean.valueOf(false)));
            p_379694_ = p_379694_.below();
        }

        p_379973_.setBlock(p_379694_, HolyBackBlocks.PALE_HANGING_MOSS.defaultBlockState().setValue(HangingMossBlock.TIP, Boolean.valueOf(true)));
    }
}
